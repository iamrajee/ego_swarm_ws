...
class PlanningVisualization:
    def __init__(self):
        ...
        self.swarm_formation_visual_pub = rospy.Publisher("comm_swarm_graph_visual_py", MarkerArray, queue_size=10)
        ...
        self.drone_all_posearray_ = rospy.Subscriber("/comm_poseArray", PoseArray, self.drone_all_posearray_callback)
    ...
    def drone_all_posearray_callback(self, msg):
        if self.formation_size_ <= 0:
            return
        self.swarm_all_posearray = msg#np.array([msg.pose.pose.position.x, msg.pose.pose.position.y, msg.pose.pose.position.z])
        self.swarm_odom = [[p.position.x, p.position.y, p.position.z] for p in msg.poses]
